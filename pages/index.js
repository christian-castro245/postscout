...
const PERM_OPTS = [
  { value: 'lesen',   label: 'Nur lesen',             desc: 'Kann Dokumente und Aufgaben sehen' },
  { value: 'abhaken', label: 'Lesen + abhaken',        desc: 'Kann Aufgaben als erledigt markieren' },
  { value: 'notizen', label: 'Lesen + abhaken + Notizen', desc: 'Kann zusätzlich Notizen hinzufügen' },
]
const FREQ_OPTS = [
  { value: 'taeglich',   label: 'Täglich' },
  { value: '2x_woche',   label: '2x pro Woche' },
  { value: 'woechentlich', label: 'Wöchentlich (Montags)' },
]
...
export default function Home() {
  ...
  // Is this user a family member viewing someone else?
  const [ownerView, setOwnerView]   = useState(null) // { ownerId, permission }
  ...
  async function init(session) {
    // Check if this user is a family member for someone
    const { data: zugaenge } = await supabase
      .from('familien_zugang')
      .select('inhaber_id, berechtigung')
      .eq('mitglied_id', session.user.id)
      .eq('aktiv', true)
      .limit(1)
  ...
  async function signOut() {
    ...
    setFamilyMembers([]); setOwnerView(null)
  }
  ...
  async function handleAuth() {
    ...
    else if (authMode === 'register') setAuthMsg({ text: 'Bestätigungs-E-Mail gesendet – bitte Postfach prüfen.', err: false })
  ...
  function addPhoto(file) {
    ...
    reader.onload = e => {
      ...
      setPhotos(prev => [...prev, { file, base64: url.split(',')[1],
        previewUrl: file.type.startsWith('image/') ? url : null, mimeType: file.type }])
    }
  ...
  async function sendInvite() {
    ...
    try {
      ...
      const { error } = await supabase.from('familien_zugang').insert({
        ...
        mitglied_email: inviteEmail,
        berechtigung: invitePerm,
        invite_token: token,
        aktiv: false,
      })
      ...
      const emailRes = await fetch('/api/invite', {
        ...
        body: JSON.stringify({
          ...
          ownerName: session.user.email.split('@')[0],
          permission: invitePerm,
          inviteToken: token,
        }),
      })
      if (!emailRes.ok) throw new Error('E-Mail konnte nicht gesendet werden')
      setInviteMsg({ text: `Einladung an ${inviteEmail} gesendet ✓`, err: false })
      setInviteEmail('')
      loadFamilyMembers(session.user.id)
    } catch (e) {
      setInviteMsg({ text: e.message, err: true })
  ...
  async function generateQR() {
    ...
    if (!error) {
      setQrToken(`${appUrl}/join?token=${token}`)
      setShowQr(true)
      loadFamilyMembers(session.user.id)
    }
  }
  ...
  async function saveReminder(updates) {
    ...
    if (error) setReminderMsg({ text: 'Fehler beim Speichern: ' + error.message, err: true })
    else setReminderMsg({ text: 'Einstellungen gespeichert ✓', err: false })
  ...
  function exportPDF() {
    const jahr = getYear()
    const filtered = docs.filter(d =>
      (d.jahr||new Date().getFullYear())===jahr&&(d.steuerrelevant||d.kategorie==='Steuer / Finanzamt')
    )
    ...
    doc.text(`Erstellt: ${new Date().toLocaleDateString('de-DE')} · ${filtered.length} Dok.`,pad,y); y+=9
    ...
    filtered.forEach((d,i)=>{
      ...
      doc.text(d.kategorie+' · '+datum+bet,pad+3,y); y+=5
    ...
    setExportMsg({ text:`✓ PDF mit ${filtered.length} Dok.`, err:false })
    setTimeout(()=>setExportMsg(null),4000)
  }
    ...
    <>
        ...
        <header className="header">
          <div className="logo">
            ...
            <div>
              ...
              <div className="logo-sub">{ownerView ? '👁 Familienansicht' : 'Briefe verstehen – nichts verpassen'}</div>
            </div>
          </div>
          {session && (
        ...
        {session && (
          <>
            <nav className="tabs">
              ...
              {isOwner && <button className={`tab ${tab==='scan'?'tab-active':''}`} onClick={()=>setTab('scan')}>Scannen</button>}
              <button className={`tab ${tab==='archiv'?'tab-active':''}`} onClick={()=>{setTab('archiv')}}>Archiv</button>
              {isOwner && <button className={`tab ${tab==='familie'?'tab-active':''}`} onClick={()=>setTab('familie')}>Familie</button>}
              {isOwner && <button className={`tab ${tab==='export'?'tab-active':''}`} onClick={()=>setTab('export')}>Export</button>}
            </nav>
              ...
              <div>
                  ...
                  <div className="mail-banner">
                    <div className="mail-banner-top">
                      ...
                      <div>
                        ...
                        <div className="mail-banner-sub">E-Mail-Vorlage wurde erstellt</div>
                      </div>
                ...
                {allTodos.length===0 ? (
                  <div className="empty">
                    ...
                    {isOwner && <button className="btn-ghost" onClick={()=>setTab('scan')} style={{marginTop:12}}>Brief scannen →</button>}
                  </div>
                ) : (
                  <>
                    <div className="todos-summary">
                      ...
                      {!canEdit && <span className="readonly-badge">👁 Nur-Lesen-Modus</span>}
                    </div>
              ...
              <div>
                ...
                {showQr && qrToken && (
                  <div className="settings-card" style={{textAlign:'center'}}>
                    <div className="settings-title" style={{marginBottom:12}}>📱 QR-Code zum Scannen</div>
                    <img
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(qrToken)}`}
                      alt="QR-Code für Einladung"
                      style={{borderRadius:8,border:'0.5px solid #e2e0d8',width:180,height:180}}
                    />
                      ...
                      <div key={m.id} className="member-row">
                        <div className="member-info">
                          ...
                          <div className="member-meta">
                            <span className={`member-status ${m.aktiv?'member-active':'member-pending'}`}>
                              {m.aktiv?'✓ Aktiv':'⏳ Ausstehend'}
                            </span>
                          </div>
                        </div>
                        <select
                          className="perm-select"
                          value={m.berechtigung}
              ...
              <div>
                ...
                <div className="settings-card" style={{marginTop:8}}>
                  ...
                  <div className="toggle-row" style={{marginTop:12}}>
                    ...
                    <button
                      className={`toggle-btn ${reminderSettings?.aktiv?'toggle-on':''}`}
                  ...
                  {reminderSettings?.aktiv && (
                    <>
                      ...
                      <div className="field-group">
                        <label className="field-label">Uhrzeit (lokale Zeit)</label>
                        <select className="year-sel" style={{width:'100%'}}
                          value={reminderSettings?.uhrzeit_utc??7}
                      ...
                      <div className="toggle-row">
                        <div>
                          ...
                          <div style={{fontSize:11,color:'#9a978e',marginTop:2}}>Nur dringende + Frist innerhalb 7 Tage</div>
                        </div>
                        <button
                          className={`toggle-btn ${reminderSettings?.nur_dringende?'toggle-on':''}`}
      ...
      <style jsx global>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .tabs{display:flex;gap:3px;margin-bottom:1.25rem;background:var(--surface2);border-radius:11px;padding:3px;overflow-x:auto}
        .tab{flex:1;min-width:fit-content;padding:8px 6px;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer;color:var(--text3);border:none;background:transparent;transition:background 140ms,color 140ms;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:4px;white-space:nowrap}
        .tab-active{background:var(--surface);color:var(--text)}
        .tab-badge{background:#e4e2d9;color:var(--text2);border-radius:20px;font-size:10px;font-weight:600;padding:1px 6px}
        ...
        .mail-banner-top{display:flex;align-items:center;gap:10px;margin-bottom:10px;font-size:22px}
        ...
        .member-meta{display:flex;gap:6px;margin-top:3px}
        .member-status{font-size:11px;padding:2px 8px;border-radius:20px}
        ...
        .toggle-row{display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-top:0.5px solid var(--border);margin-top:8px}
        ...
        .toggle-knob{width:18px;height:18px;border-radius:50%;background:#fff;position:absolute;top:3px;left:3px;transition:transform 200ms var(--ease-out)}
        ...
        @media(prefers-reduced-motion:reduce){
          .todo-card,.tab,.btn-primary,.btn-secondary,.btn-ghost,.upload-zone,.chip,.todo-check,.toggle-btn,.toggle-knob{transition:none}
        }
      `}</style>
    </>
  )
